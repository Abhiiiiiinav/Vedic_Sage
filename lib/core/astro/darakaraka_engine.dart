...
class JaiminiKaraka {
  ...
  @override
  String toString() => '$karakaName: $planet at ${degree.toStringAsFixed(2)}° in $signName (House $houseNumber)';
}
...
class DarakarakaEngine {
  ...
  static List<JaiminiKaraka> calculateCharakarakas({
    ...
    for (final abbr in includePlanets) {
      double? longitude;
      final fullName = planetNames[abbr] ?? abbr; // Get full name like "Sun" from "Su"
    ...
    for (int i = 0; i < planetDegreesList.length && i < karakaOrder.length; i++) {
  ...
  static JaiminiKaraka? getKarakaByName(List<JaiminiKaraka> karakas, String name) {
    try {
  ...
  static Map<String, dynamic> analyzeDKLagnaRelation(Map<String, dynamic> chartData) {
    ...
    if (isInTrine) {
      relation = 'Harmonious';
      interpretation = 'Partner naturally supports your life path and identity. Relationship flows with dharmic alignment.';
    } else if (isInKendra) {
      relation = 'Significant';
      interpretation = 'Partner has major influence on your life direction. Strong karmic connection with active engagement.';
    } else if (isInDustana) {
      relation = 'Challenging';
      interpretation = 'Relationship requires conscious effort. Growth through working through difficulties together.';
    } else {
      relation = 'Moderate';
      interpretation = 'Partner influence is present but not overwhelming. Balance of support and independence.';
    }
